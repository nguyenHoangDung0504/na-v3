...
async function execute() {
	...
	// await copy(`_(${rs.code}, "${rs.rjCode}", "${rsCVstr}", "${rsTagStr}", "", "${rs.engName}", "${rs.japName}", t0i0a)`)
	await copy(`${rs.code}:\n\t${rs.rjCode}\n\t${rsCVstr}\n\t${rsTagStr}\n\t\n\t${rs.engName}\n\t${rs.japName}\n\tt0i0a`)
}
...
async function initData() {
	...
	const data = await fetch('http://127.0.0.1:5500/.data_compressor/storage/data.csv')
		...
		.then((data) => data.map((line) => line.toSpliced(5)))
...
function getConstKeys() {
	const TAG_KEYS = new Map([
		...
		['Mommie', 'Mother,Milf,Mature Woman'],
		['Mother', 'Mother,Milf,Mature Woman'],
