...
export default class ListView {
	...
	constructor(ModelType, listContainer, itemRenderer) {
		if (document.readyState === 'loading')
			console.warn('--> [ListView.constructor]: Warning, document is still loading. Render might fail.');
		...
		if (this._container.childElementCount < 1)
			throw new Error('--> [ListView.constructor]: Error, cannot not find a valid HTML template inside the container.');
		if (typeof this._renderItem !== 'function')
			throw new TypeError("--> [ListView.constructor]: Error, 'itemRenderer' must be a valid function.");

		/**@private */ this._template = this._container.firstElementChild?.cloneNode(true);
	...
	update(newData) {
		if (!Array.isArray(newData)) {
			throw new Error(`--> [ListView.updateData]: Error, 'newData' must be an array.`);
		}
	...
	_render(newData = this._data) {
		this._eventHandlers['before-render'].forEach((cb) => cb(this._container, newData));
		...
		newData.forEach((item, index) => {
			...
			this._eventHandlers['before-item-added'].forEach((cb) => cb(node, item, index));
			fragment.appendChild(node);
			this._eventHandlers['after-item-added'].forEach((cb) => cb(node, item, index));
		});

		oldNodes.forEach((node) => {
			// Duyệt qua danh sách các key đã lưu
			const boundItem = this._itemCacheKeys.find((key) => this._itemCache.get(key) === node);
			if (boundItem && !newData.includes(boundItem)) {
				...
				this._itemCacheKeys = this._itemCacheKeys.filter((key) => key !== boundItem);
			}
		});
		...
		/**@private */
		this._data = newData;
		this._eventHandlers['after-render'].forEach((cb) => cb(this._container, this._data));
	}
