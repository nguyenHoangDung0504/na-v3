...
/* ═══════════════════════════════════════════════════════
	...
	FIX: dùng ref array thay vì querySelector mỗi frame
	FIX: scrollIntoView thay vì offsetTop (tránh forced reflow)
	══════════════════════════════════════════════════════ */
const SubtitlePanel = (() => {
	...
	/* ── Ref trực tiếp đến từng DOM element → không cần querySelector ── */
	let itemEls = [] // FIX: array thay vì querySelector mỗi lần
	let activeEl = null // FIX: ref element đang active
	let activeCueIdx = -1
	let onClickCb = null
	...
	function updateActive(idx) {
		...
		// Bỏ active cũ — trực tiếp qua ref
	...
	/* ── Scroll đến cue active trong wrap, không ảnh hưởng trang ngoài ── */
	// KHÔNG dùng scrollIntoView: nó scroll cả viewport ngoài, gây UI nhảy khi mở panel
	// KHÔNG đọc offsetTop ngay lập tức khi panel transition chưa xong → dùng requestAnimationFrame
	function scrollToActive(smooth = true, deferred = false) {
		...
		deferred ? requestAnimationFrame(() => requestAnimationFrame(doScroll)) : doScroll()
	}
...
const PlaylistUI = (() => {
	...
	function setActive(idx) {
		list.querySelectorAll('.pl-item').forEach((el, i) => el.classList.toggle('active', i === idx))
		list.querySelector('.pl-item.active')?.scrollIntoView({ block: 'nearest', behavior: 'smooth' })
	}
...
/* ═══════════════════════════════════════════════════════
	...
	FIX: probe tuần tự thay vì Promise.all → giảm RAM đột biến
	FIX: Audio element được GC ngay sau mỗi probe
	══════════════════════════════════════════════════════ */
const VTTAdaptive = (() => {
	...
	/* ── Probe 1 audio element tại một thời điểm ── */
	...
	/* ── FIX: probe tuần tự thay vì Promise.all
		 Lần đầu chậm hơn đôi chút, nhưng chỉ có 1 Audio element
		 trong RAM tại một thời điểm → không gây OOM trên mobile ── */
	...
	async function loadForTrack(track, tracks, idx) {
		if (hasAllVtt) {
			...
			durationCache.set(track.audioURL, duration)
			const offset = await computeOffset(tracks, idx)
			return sliceCues(offset, duration)
		}

		if (track.vttURL) {
			return VTT.load(track.vttURL)
		}

		return []
	}

	/* ── Cache duration từ audio element chính (đã load sẵn) ── */
...
/* ═══════════════════════════════════════════════════════
	...
	Điều phối audio, phụ đề, ảnh và tất cả UI
	══════════════════════════════════════════════════════ */
const Player = (() => {
	/* ── DOM refs ── */
	...
	function _bindFloatSubDrag() {
		...
		floatSub.addEventListener('pointerdown', (e) => {
			...
			floatSub.style.top = rect.top - stageRect.top + 'px'
		...
		floatSub.addEventListener('pointermove', (e) => {
			if (!dragging) return
			const stageRect = document.getElementById('stage').getBoundingClientRect()
			const maxY = stageRect.height - floatSub.offsetHeight
			const newTop = Math.max(0, Math.min(maxY, e.clientY - stageRect.top - offsetY))
			floatSub.style.top = newTop + 'px'
		})

		floatSub.addEventListener('pointerup', () => {
			dragging = false
		})
	}
	...
	function onMetadataLoaded() {
		...
		// Cache duration cho VTTAdaptive dùng — không cần probe lại
	...
	/* ── Cập nhật phụ đề theo thời gian hiện tại ── */
	function _updateSubtitle(t) {
		if (!cues.length) {
			// FIX: chỉ write DOM khi cần
		...
		// FIX: chỉ write textContent khi text thực sự thay đổi
		if (floatSubOn && floatSubText.textContent !== text) {
			floatSubText.textContent = text
		}
		...
		if (found !== activeCueIdx) {
			...
			if (found >= 0) SubtitlePanel.scrollToActive()
		}
	}
...
if (!VTTAdaptive.isAllVtt()) {
	// Fallback: gán vttURL riêng từng track
	TRACKS.forEach((t) => {
...
/* ═══════════════════════════════════════════════════════
	FILE UPLOAD
	FIX: revokeObjectURL sau khi Slideshow đã nhận src
	══════════════════════════════════════════════════════ */
...
document.getElementById('file-input').addEventListener('change', (e) => {
	...
	// FIX: revoke sau 1 tick để browser kịp gán src vào img/video element
