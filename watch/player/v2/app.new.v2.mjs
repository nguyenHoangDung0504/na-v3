...
const SubtitlePanel = (() => {
	...
	/* ── Scroll đến cue active — dùng scrollIntoView, không đọc offsetTop ── */
	function scrollToActive(smooth = true) {
		if (!activeEl || userScrolling) return
		// FIX: scrollIntoView không trigger forced reflow như offsetTop
		activeEl.scrollIntoView({ block: 'nearest', behavior: smooth ? 'smooth' : 'auto' })
	}
